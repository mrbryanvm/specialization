"""
server.py — RAG-Powered WhatsApp AI Sales Assistant with Lead Scoring
=====================================================================

Day 14: Evolution of Day 13's RAG bot.
NEW FEATURES:
  - Lead Scoring: Every conversation is evaluated in real time (1-10)
  - Structured Data Extraction: The LLM outputs a Pydantic model (LeadData)
    with the lead's name, budget, product interest, timeline, and score.
  - Score Labels: HOT (8-10), WARM (5-7), COLD (1-4)
  - The bot asks qualifying questions NATURALLY (not robotic).
...
ARCHITECTURE:
    ...
    Lead data updated in memory (lead_tracker)
...
app = FastAPI(
    title="Mueblería América — AI Sales Assistant with RAG + Lead Scoring",
    description=(
        "WhatsApp AI agent that searches the real business catalog and "
        "qualifies leads with intelligent scoring (1-10)."
    ),
    version="3.0.0",
)
...
# ──────────────────────────────────────────────────────────────
# 5. CONVERSATIONAL MEMORY + LEAD TRACKER
# ──────────────────────────────────────────────────────────────
...
def get_ai_response(sender_number: str, user_input: str) -> str:
    """
    The AI brain with RAG + Lead Scoring. For each message:
...
# ──────────────────────────────────────────────────────────────
# 7. HEALTH CHECK & LEAD DASHBOARD ENDPOINT
# ──────────────────────────────────────────────────────────────
...
async def health_check():
    ...
    return {
        ...
        "version": "3.0.0 (RAG + Lead Scoring)",
        "model": "llama-3.1-8b-instant",
...
# ──────────────────────────────────────────────────────────────
# 8. LEAD DASHBOARD — View all scored leads
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# 9. TEST ENDPOINT — Quick way to test RAG + Scoring
# ──────────────────────────────────────────────────────────────
...
async def test_rag(q: str = "¿Qué productos tienen?"):
    """
    Test the RAG + Lead Scoring pipeline from the browser.
...
# ──────────────────────────────────────────────────────────────
# 10. RESET ENDPOINT — Clear a conversation for testing
# ──────────────────────────────────────────────────────────────
...
#   5. Reset test:       curl -X DELETE http://localhost:8000/reset/test_user_browser
# ──────────────────────────────────────────────────────────────
